...
export function useModelSelector() {
  ...
  useEffect(() => {
    if (modelType === 'gpt') {
      ...
      localStorage.setItem('aiAssistantSelectedModel', GEMINI_MODELS[0].value);
    } else if (modelType === 'claude') {
      setSelectedModel(CLAUDE_MODELS[0].value);
      localStorage.setItem('aiAssistantSelectedModel', CLAUDE_MODELS[0].value);
