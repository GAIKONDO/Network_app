...
export async function saveStartup(startup: Partial<Startup>): Promise<string> {
  try {
    const startupId = startup.id || generateStartupId();
    ...
    try {
      ...
      if (existingDoc.exists()) {
        ...
        // evaluationChartとevaluationChartSnapshotsがnullまたはundefinedの場合、既存データを保持
        // ただし、既存データがJSON文字列の場合はパースする
...
export async function getAllTopicsBatch(): Promise<TopicInfo[]> {
  try {
    ...
    // Graphvizカードのトピックを取得（SQLiteのtopicsテーブルから）
    // 注意: sourceTypeカラムは存在しないため、meetingNoteIdが'graphviz_'で始まるものをフィルタリング
    try {
      const { callTauriCommand } = await import('./localFirebase');
      // すべてのtopicsを取得（条件なし）
      ...
      // meetingNoteIdが'graphviz_'で始まるものをフィルタリング
      ...
      for (const item of graphvizTopics) {
        ...
        allTopics.push({
          ...
          meetingNoteTitle: topicData.title, // Graphvizカードの場合はタイトルをそのまま使用
          itemId: '', // GraphvizカードにはitemIdがない
          organizationId: topicData.organizationId || '',
          companyId: topicData.companyId || undefined,
          topicDate: undefined, // Graphvizカードには日付がない
          isAllPeriods: true, // Graphvizカードは全期間に反映
