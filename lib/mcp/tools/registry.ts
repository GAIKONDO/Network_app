...
class UpdateCauseEffectDiagramTool implements MCPToolImplementation {
  ...
  arguments = [
    ...
    { name: 'modelType', type: 'string' as const, description: 'モデルタイプ（gpt/local/gemini/claude）', required: false, default: 'gpt' },
  ...
  async execute(request: MCPToolRequest): Promise<MCPToolResult> {
    ...
    try {
      ...
      const finalModelType = (modelType || 'gpt') as 'gpt' | 'local' | 'gemini' | 'claude';
      const finalSelectedModel = selectedModel || 'gpt-5-mini';
...
export function registerStandardTools(): void {
  ...
  console.log('[MCPTools] 標準Toolを登録しました（Agent関連Tool、議事録整形Tool、組織作成Toolを含む）');
}
