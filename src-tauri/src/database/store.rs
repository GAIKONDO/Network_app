...
const ALLOWED_TABLES: &[&str] = &[
    ...
    "topicFiles", // トピックファイルテーブル
    "graphvizYamlFiles", // Graphviz YAMLファイルテーブル
    "graphvizDotFiles", // Graphviz DOTファイルテーブル
    "graphvizYamlFileAttachments", // Graphviz YAMLファイル添付ファイルテーブル
];
...
/// 議事録の特定アイテムのcontentを直接SQLで更新
/// SQLiteのjson_set関数を使用して、JSON内の特定パスを更新します
pub fn update_meeting_note_item_content(
    meeting_note_id: &str,
    tab_id: &str,
    item_id: &str,
    new_content: &str,
) -> SqlResult<()> {
    eprintln!("🔧 [update_meeting_note_item_content] 開始: meeting_note_id={}, tab_id={}, item_id={}", 
        meeting_note_id, tab_id, item_id);
    
    let db = get_db().ok_or_else(|| {
        eprintln!("❌ [update_meeting_note_item_content] データベースが初期化されていません");
        rusqlite::Error::SqliteFailure(
            rusqlite::ffi::Error::new(rusqlite::ffi::SQLITE_MISUSE),
            Some("データベースが初期化されていません".to_string())
        )
    ...
    // まず、現在のcontentを取得して、アイテムが存在するか確認
    let current_content: Option<String> = tx.query_row(
        "SELECT content FROM meetingNotes WHERE id = ?1",
        [meeting_note_id],
        |row| row.get(0),
    )?;
    ...
    if current_content.is_none() {
        return Err(rusqlite::Error::SqliteFailure(
            ...
            Some(format!("議事録が見つかりません: {}", meeting_note_id))
        ));
    }
    
    // 配列内のアイテムのインデックスを探す
    // まず、配列の長さを取得
    let items_path = format!("$.{}.items", tab_id);
    let meeting_note_id_str = meeting_note_id.to_string();
    
    eprintln!("🔍 [update_meeting_note_item_content] アイテム検索開始: tab_id={}, item_id={}", tab_id, item_id);
    
    let array_length: i64 = tx.query_row(
        r#"
        SELECT json_array_length(json_extract(content, ?1))
        FROM meetingNotes 
        WHERE id = ?2
        "#,
        [items_path.clone(), meeting_note_id_str.clone()],
        |row| row.get(0),
    ).unwrap_or(0);
    
    eprintln!("🔍 [update_meeting_note_item_content] 配列の長さ: {}", array_length);
    
    // 配列内の各要素をチェックして、item_idが一致するインデックスを探す
    let mut item_index: Option<i64> = None;
    for i in 0..array_length {
        let item_id_path = format!("$.{}.items[{}].id", tab_id, i);
        let current_item_id: Option<String> = tx.query_row(
            r#"
            SELECT json_extract(content, ?1)
            FROM meetingNotes 
            WHERE id = ?2
            "#,
            [item_id_path.clone(), meeting_note_id_str.clone()],
            |row| row.get(0),
        ).ok().flatten();
        
        eprintln!("🔍 [update_meeting_note_item_content] インデックス {}: item_id={:?}", i, current_item_id);
        
        if let Some(id) = current_item_id {
            if id == item_id {
                item_index = Some(i);
                eprintln!("✅ [update_meeting_note_item_content] アイテムが見つかりました: インデックス={}", i);
                break;
            }
        }
    }
    ...
    if item_index.is_none() {
        eprintln!("❌ [update_meeting_note_item_content] アイテムが見つかりません: item_id={}, tab_id={}", item_id, tab_id);
        return Err(rusqlite::Error::SqliteFailure(
            rusqlite::ffi::Error::new(rusqlite::ffi::SQLITE_NOTFOUND),
            Some(format!("アイテムが見つかりません: item_id={}, tab_id={}", item_id, tab_id))
        ));
    }
    
    let index = item_index.unwrap();
    let content_path = format!("$.{}.items[{}].content", tab_id, index);
    
    eprintln!("🔧 [update_meeting_note_item_content] 更新パス: {}", content_path);
    eprintln!("🔧 [update_meeting_note_item_content] 新しいコンテンツの長さ: {}", new_content.len());
    eprintln!("🔧 [update_meeting_note_item_content] 新しいコンテンツの先頭100文字: {}", 
        new_content.chars().take(100).collect::<String>());
    
    // json_setを使用してcontentを更新
    // SQLiteのjson_setは、JSON文字列を更新して返します
    // new_contentをJSON文字列としてエスケープする必要があります
    // json_quote関数を使用して、文字列をJSON文字列としてエスケープします
    let updated_content: String = match tx.query_row(
        r#"
        SELECT json_set(content, ?1, json_quote(?2))
        FROM meetingNotes 
        WHERE id = ?3
        "#,
        [content_path.clone(), new_content.to_string(), meeting_note_id_str.clone()],
        |row| row.get::<usize, String>(0),
    ) {
        Ok(content) => {
            eprintln!("✅ [update_meeting_note_item_content] json_set成功: 更新後のコンテンツの長さ={}", content.len());
            content
        },
        Err(e) => {
            eprintln!("❌ [update_meeting_note_item_content] json_setエラー: {}", e);
            return Err(e);
        }
    };
    ...
    let rows_affected = tx.execute(
        "UPDATE meetingNotes SET content = ?1, updatedAt = ?2 WHERE id = ?3",
        [updated_content.clone(), now, meeting_note_id_str.clone()],
    )?;
    
    eprintln!("✅ [update_meeting_note_item_content] UPDATE実行: 影響を受けた行数={}", rows_affected);
    
    // 更新後の内容を確認
    let verify_content: Option<String> = tx.query_row(
        r#"
        SELECT json_extract(content, ?1)
        FROM meetingNotes 
        WHERE id = ?2
        "#,
        [content_path, meeting_note_id_str],
        |row| row.get::<usize, String>(0),
    ).ok();
    
    if let Some(verified) = verify_content {
        eprintln!("✅ [update_meeting_note_item_content] 更新確認: 更新後のコンテンツの長さ={}", verified.len());
        eprintln!("✅ [update_meeting_note_item_content] 更新後のコンテンツの先頭100文字: {}", 
            verified.chars().take(100).collect::<String>());
    } else {
        eprintln!("⚠️ [update_meeting_note_item_content] 更新確認: コンテンツが取得できませんでした");
    }
    
    tx.commit()?;
    eprintln!("✅ [update_meeting_note_item_content] 成功: item_id={}, content_length={}", 
        item_id, new_content.len());
