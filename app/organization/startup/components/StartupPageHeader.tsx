...
import type { Startup, OrgNodeData } from '@/lib/orgApi';
...
interface StartupPageHeaderProps {
...
export default function StartupPageHeader({
  orgData,
  startup,
  organizationId,
  allOrganizations,
  ...
  // デバッグ用: allOrganizationsの状態を確認
  React.useEffect(() => {
    console.log('📊 [StartupPageHeader] allOrganizations:', {
      count: allOrganizations.length,
      organizations: allOrganizations.map(org => ({ id: org.id, name: org.name, title: org.title })),
      currentOrganizationId: organizationId,
      selectValue,
      isChangingOrganization,
      savingStatus,
      isDisabled: isChangingOrganization || savingStatus === 'saving',
    });
  }, [allOrganizations, organizationId, selectValue, isChangingOrganization, savingStatus]);

  const handleOrganizationChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    const newOrganizationId = e.target.value;
    console.log('🔄 [StartupPageHeader] handleOrganizationChange called:', {
    ...
    if (!newOrganizationId || newOrganizationId === organizationId) {
      // 同じ組織が選択された場合は何もしない
      console.log('⚠️ [StartupPageHeader] 同じ組織が選択されました');
      return;
    }
  ...
  const handleConfirmOrganizationChange = async () => {
    ...
    try {
      console.log('✅ [StartupPageHeader] 組織変更を開始:', newOrganizationId);
      setIsChangingOrganization(true);
      await onOrganizationChange(newOrganizationId);
      // 成功した場合は、ページ遷移が行われるため、ここでは何もしない
      console.log('✅ [StartupPageHeader] 組織変更が完了しました');
    } catch (error: any) {
      console.error('❌ [StartupPageHeader] 組織変更エラー:', error);
