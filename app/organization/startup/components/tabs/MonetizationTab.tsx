    ...
    <div style={{ padding: '24px' }}>
      ...
      <div style={{ marginBottom: '16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          ...
          {startup?.monetizationDiagramId && (
            <a
              href={`#monetization-${startup.monetizationDiagramId}`}
              onClick={(e) => {
                e.preventDefault();
                navigator.clipboard.writeText(startup.monetizationDiagramId || '');
                alert(`マネタイズ図ID "${startup.monetizationDiagramId}" をクリップボードにコピーしました`);
              }}
              style={{
                fontSize: '12px',
                color: '#3B82F6',
        ...
        <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
          <button
            onClick={async () => {
              ...
              if (!startup.monetizationDiagramId) {
          ...
          {localMonetizationDiagram ? (
            <div style={{ border: '1px solid #E5E7EB', borderRadius: '6px', padding: '20px', backgroundColor: '#FFFFFF' }}>
              <MermaidDiagram
                ...
                diagramId={`monetization-${startupId}`}
              />
            </div>
          ) : (
            <div style={{ 
