    ...
    <div style={{ padding: '24px' }}>
      ...
      <div style={{ marginBottom: '16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          ...
          {startup?.relationDiagramId && (
            <a
              href={`#relation-${startup.relationDiagramId}`}
              onClick={(e) => {
                e.preventDefault();
                navigator.clipboard.writeText(startup.relationDiagramId || '');
                alert(`相関図ID "${startup.relationDiagramId}" をクリップボードにコピーしました`);
              }}
              style={{
                fontSize: '12px',
                color: '#3B82F6',
