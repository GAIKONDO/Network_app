...
import { findOrganizationById, getMeetingNoteById, getTopicsByMeetingNote, getStartupById, type OrgNodeData, type TopicInfo, type MeetingNote, type Startup } from '@/lib/orgApi';
      ...
      try {
        ...
        if (updatedStartup) {
          devLog('📖 [モーダル保存] 再読み込み完了:', {
            topicIdsLength: updatedStartup.topicIds?.length || 0,
          });
          setStartup(updatedStartup);
          setLocalTopicIds(Array.isArray(updatedStartup.topicIds) ? updatedStartup.topicIds : []);
        }
      } catch (reloadError) {
