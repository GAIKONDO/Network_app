...
import { saveRegulation } from '@/lib/orgApi';
...
interface UseTopicManagementProps {
  monthContents: RegulationData;
  setMonthContents: (contents: RegulationData) => void;
  activeTab: TabType;
...
export function useTopicManagement({
  ...
  const [similarTopics, setSimilarTopics] = useState<Array<{ topicId: string; regulationId: string; similarity: number }>>([]);
  ...
  const confirmDeleteTopic = useCallback(async () => {
    ...
    if (regulation && organizationId) {
      try {
        ...
        await retryDbOperation(() => saveRegulation({
          ...regulation,
          content: contentJson,
        }));
  ...
  }, [deleteTargetTopicItemId, deleteTargetTopicId, monthContents, activeTab, setMonthContents, setHasUnsavedChanges, regulation, organizationId, regulationId, setSavingStatus, retryDbOperation]);
