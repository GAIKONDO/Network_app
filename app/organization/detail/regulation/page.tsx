...
function RegulationDetailPageContent() {
  ...
  const regulationId = searchParams?.get('regulationId') as string;
  ...
  // タブの順番管理（useRegulationDataの前に定義する必要がある）
  ...
  useEffect(() => {
    if (regulation?.content) {
      try {
        const parsed = JSON.parse(regulation.content) as RegulationData & { 
  ...
  // ナビゲーションで展開されている制度アイテムを管理（アイテムIDをキーとする）
    ...
    for (const [tabKey, tabData] of Object.entries(monthContents)) {
      ...
      for (const item of tabData.items) {
        ...
        if (foundTopic) {
          ...
          router.replace(`/organization/detail/regulation${newUrl}`, { scroll: false });
  ...
  const confirmDeleteItem = async () => {
    ...
    // JSONファイルに自動保存
    if (regulation && updatedContents) {
      try {
        devLog('💾 [confirmDeleteItem] 保存開始...');
        const contentJson = JSON.stringify(updatedContents, null, 2);
        // 事業会社の管理はorganizationsテーブルのtypeカラムで行うため、通常のsaveRegulationを使用
  ...
  }, [regulation, monthContents, setMonthContents, setActiveSection, setEditingMonth, setEditingSection, setEditingContent, setEditingItemTitle, setHasUnsavedChanges]);
    ...
    return (
      <Layout>
        <div style={{ padding: '40px' }}>
          <h2 style={{ marginBottom: '8px' }}>制度詳細</h2>
          ...
          <button
            onClick={async () => {
              ...
              router.push(`/organization/detail?id=${organizationId}&tab=regulations`);
            }}
            style={{
              marginTop: '16px',
              padding: '8px 16px',
          ...
          onSave={async () => {
            ...
            if (regulation) {
              try {
                ...
                const updatedNote: Regulation = {
                  ...regulation,
                  content: JSON.stringify(dataToSave),
                };
                await saveRegulation(updatedNote);
        ...
        onConfirm={async () => {
          ...
          try {
            ...
            if (pendingEntities && pendingEntities.length > 0) {
              ...
              const topicEmbeddingId = `${regulationId}-topic-${editingTopicId}`;
      ...
      <TopicModal
        ...
        onSave={async (updatedContents) => {
          try {
            ...
            // topicsレコードを作成または確認
            const topicEmbeddingId = `${regulationId}-topic-${topicId}`;
            try {
            ...
            if (entitiesToSave && entitiesToSave.length > 0) {
              ...
              for (const entity of entitiesToCreate) {
                try {
                  ...
                  const orgId = entity.organizationId || organizationId;
                  ...
                  if (!sourceEntityExists || !targetEntityExists) {
                    ...
                    if (sourceEntity && targetEntity) {
                      ...
                      if (fallbackSourceId && fallbackTargetId) {
                        const orgId = relation.organizationId || organizationId;
