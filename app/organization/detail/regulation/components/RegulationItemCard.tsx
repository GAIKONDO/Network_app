...
interface RegulationItemCardProps {
  ...
  onSetMonthContents: (contents: RegulationData) => void;
...
// 制度アイテムカードの範囲を特定するためのヘルパー関数
export function getRegulationItemCardRange() {
  // この関数は使用されませんが、コードの範囲を明確にするために残します
  return { start: 1324, end: 3022 };
}

export default function RegulationItemCard({
  item,
  activeTab,
  editingMonth,
  editingSection,
              ...
              try {
                ...
                cleanedSections.forEach((section) => {
                  ...
                  newTopics.push({
                    ...
                    mentionedDate: item.date || undefined, // 親の制度の日時を引き継ぐ
                    createdAt: now,
                    updatedAt: now,
                  });
                });
                      ...
                      try {
                        ...
                        if (topicIndex !== -1) {
                          ...
                          try {
                            ...
                            console.error(`❌ [RegulationItemCard] Topicsテーブルへの保存失敗: ${topic.id}`, {
                              error: saveError,
                          ...
                          // リレーションを保存
                          const topicEmbeddingId = `${regulationId}-topic-${topic.id}`;
