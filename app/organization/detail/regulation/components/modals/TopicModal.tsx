...
import type { TabType, RegulationData } from '../../types';
...
interface TopicModalProps {
  ...
  // Callbacks
  onClose: () => void;
  onSave: (updatedContents: RegulationData) => void;
  onCancel?: () => void;
...
export default function TopicModal({
  ...
  const loadTopicFiles = async () => {
    if (!editingTopicId || !regulationId) return;
    setIsLoadingFiles(true);
    try {
      const files = await getTopicImagePaths(editingTopicId, regulationId);
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file || !editingTopicId || !regulationId) return;
  ...
  const handleConfirmDeleteFile = async () => {
    if (!fileToDelete || !editingTopicId || !regulationId) return;

    try {
      await deleteTopicImage(organizationId, editingTopicId, regulationId, fileToDelete.path);
      await loadTopicFiles();
  ...
  const handleGenerateMetadata = async () => {
    ...
    try {
      ...
      const topicEmbeddingId = editingTopicId 
        ? `${regulationId}-topic-${editingTopicId}`
        : `${regulationId}-topic-${generateUniqueId()}`;
